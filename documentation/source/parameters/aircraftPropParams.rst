...
``mountOrientation``: The rotation Roll-Pitch-Yaw (in this sequence) rotation angles which rotate the thruster body from the Body Frame to its actual `Propeller Frame <../theory_elements/referenceFrames.html>`_ fixture. Since thrust is mostly generated towards the positive-x direction of the `Propeller Frame <../theory_elements/referenceFrames.html>`_, for a simplified airplane motor installation on the CoG, this should be [0.0,0.0,0.0].
Care is needed at this point, in case this thruster is gimballed. last_letter gimbals thrusters around the `Propeller Frame <../theory_elements/referenceFrames.html>`_ z-axis. Consequently, ``mountOrientation`` should be selected in such a way that gimballing will end-up occuring around the correct axis.
...
More information on the related parameters can be found in the `polynomial parameters page <polynomialParams.html>`_.
...
``propPowerPoly``: A 1-variable (1D) polynomial describing the power coefficient of the propeller (non-dimensional) as a function of the *advance ratio*. This is a parameter group and all the related parameters must be set as a parameter under this group.
More information on the related parameters can be found in the `polynomial parameters page <polynomialParams.html>`_.
