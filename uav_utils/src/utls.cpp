...
void
quat2eulerzyx (const geometry_msgs::Quaternion q,
           double *psi, double *theta, double *phi)
{
  ...
  if (test < -tol) // test ~= -1/2 => sin(theta) ~= +1 => theta = +pi/2
    {
      *phi = 0.0;
      *theta = 0.5 * M_PI;
      // *psi = 2.0 * atan2 (q0, q3);
      *psi = 2.0 * atan2 (2.0*(q12-q03), q00-q11+q22-q33);
      return;
    }
  else if (test > +tol) // test ~= +1/2 => sin(theta) ~= -1 => theta = -pi/2
    {
      *phi = 0.0;
      *theta = -0.5 * M_PI;
      // *psi = 2.0 * atan2 (q0, q3);
    ...
    {
      *phi = atan2 (2.0*(q01 + q23), q00 - q11 - q22 + q33);
      *theta = asin (-2.0*test);
      *psi = atan2 (2.0*(q12 + q03), q00 + q11 - q22 - q33);
      return;
    }
}
